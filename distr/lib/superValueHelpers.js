...
import { All_TYPES, SIMPLE_TYPES, SUPER_VALUES, } from '../types/valueTypes.js';
...
export function checkArrayDefinition(definition) {
    if (!definition)
        return;
    const { type, default: defaultValue, defaultArray, nullable, } = definition;
    if (defaultArray) {
...
export function checkValueBeforeSet(isInitialized, definition, key, value, ignoreRo) {
    if (!isInitialized)
        throw new Error(`Init it first`);
    else if (!definition)
        throw new Error(`Doesn't have definition for key ${key}`);
 ...
 */
export function resolveNotSuperChild(definition, initialValue) {
...
export function makeNewSuperValueByDefinition(definition, childKeyOrIndex) {
    const superChildType = definition.type;
