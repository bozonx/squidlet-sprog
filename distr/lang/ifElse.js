...
export function ifElse(scope) {
    return async (p) => {
        let switchPrimitive;
        if (p.switch)
            switchPrimitive = await scope.$resolve(p.switch);
        for (const itemIndex in p.items) {
            ...
            if (!item.lines) {
                ...
                if (resolvedCase !== switchPrimitive)
                    continue;
            }
...
async function executeLines(scope, item) {
    if (!item.lines.length)
        return;
    // new scope layer for block of code
    const exeScope = newScope(undefined, scope);
